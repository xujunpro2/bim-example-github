...
export default {
	...
	methods: {
        ...
        addHeatmapGrid(){
            ...
            //设置网格化参数
            let param = {
                config:{count:100,power:5,sampleCount:12},
                sampelPoints:[
                    {x:10,y:10,value:0},
                    {x:20,y:20,value:0},
                    {x:30,y:30,value:0},
            ...
            let matrix1 = plugin.getGridMatrix(param1);
            //plugin.drawGrid(matrix1,false);
            ...
            let matrix2 = plugin.getGridMatrix(param2);
            

            let mergeMatrix = plugin.mergeGridMatrix([matrix,matrix1,matrix2]);
            plugin.drawGridTimeline(mergeMatrix,true);
            //拔高一下网格图的高度
            plugin.lineGroup.position.set(-50,20,-50)
        },
